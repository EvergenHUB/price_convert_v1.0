import pandas as pd
# Цикл на обход и конвертацию всех файлов по именам
for file_name in ['1.xlsx','2.xlsx','3.xlsx','4.xlsx',
                  '5.xlsx','6.xlsx','7.xlsx','8.xlsx',
                  '9.xlsx','10.xlsx','11.xlsx','12.xlsx']:
    for_name = pd.ExcelFile(file_name)  # Получаем файл
    sh_name = for_name.sheet_names[0]  # Получаем имена листов, оставляем только первый
    sh_new_name = sh_name.replace(' ', '')  # Убираем пробелы
    sh_new_name_2 = sh_new_name.replace('"', '')  # Убираем кавычки
    
    tabs_1 = pd.read_excel(file_name, usecols="B: BB")  # Читаем файл с пропуском первого столбца
    znachenia = tabs_1.iloc[1]  # Получаем первую сточку с заголовками
    okruglenie = znachenia.astype(int)  # Округляем цифры
    sobr_tab = (
        pd.DataFrame(list(okruglenie.items()), columns=['Name', 'Price']))  # формируем таблицу с двумя столбцами
    get_id = tabs_1.head(0)  # получаем заголовки для перевода в id
    sobr_tab['id'] = list(get_id)  # Создаем доп столбец ставим в него названия моделей из заголовков

    # Меняем имена товаров на id товаров в созданом столбце
    zamena_na_id = sobr_tab.replace({'id': {"Аквамарин": 9205, "Алтай": 9241, "Альба": 9223, "Альба СТ": 9317,
    ...
    # Сохраняем файл с новым именем на основе старого, убираем колонку с индексами
    zamena_na_id.to_excel(sh_new_name_2 + "_id_new.xlsx", sheet_name='Цены и id', index=False)
print('New files Done!')  # Удача!
